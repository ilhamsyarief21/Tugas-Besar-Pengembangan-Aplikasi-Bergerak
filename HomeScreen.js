...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    const loadFontAndFetchCategories = async () => {
      ...
      try {
        const categoriesUrl = 'http://si-sdm.id/ecourse/api/web/v1/course-categories/all';
        const categoriesResponse = await fetch(categoriesUrl);
        const categoriesResult = await categoriesResponse.json();

        if (Array.isArray(categoriesResult.items)) {
          setCategories(categoriesResult.items);
        } else {
          console.error('API response does not contain an array of items:', categoriesResult);
          setCategories([]);
        }
      } catch (error) {
    ...
    loadFontAndFetchCategories();
  }, []);

  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const coursesUrl = 'http://si-sdm.id/ecourse/api/web/v1/courses/all';
  ...
  const handleItemClick = async (index) => {
    ...
    try {
      const coursesUrl = `http://si-sdm.id/ecourse/api/web/v1/courses/all-filter-by-category?id_category=${categoryId}`;
      const coursesResponse = await fetch(coursesUrl);
      const coursesResult = await coursesResponse.json();
